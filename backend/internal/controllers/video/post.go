...
func PostVoteUp(c *fiber.Ctx) error {
	...
	defer db.Close()
	// * END(DB BLOCK)
	if err := qtx.UpdateVoteUpVideo(ctx, id); err != nil {
...
func PostVoteDown(c *fiber.Ctx) error {
	idString := c.Params("id")
	id, err := strconv.ParseInt(string(idString), 10, 64)
	if err != nil {
		log.WithField("err", err).Debug("ID could not be parsed")
	...
	defer db.Close()
	// * END(DB BLOCK)
	if err := qtx.UpdateVoteDownVideo(ctx, id); err != nil {
		log.WithField("err", err.Error()).Error("Comment could not be added")
		return c.SendStatus(fiber.StatusInternalServerError)
	}
	return c.SendStatus(fiber.StatusOK)
